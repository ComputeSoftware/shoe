...
(use-fixtures :each t/temp-dir-fixture)

(deftest merge-edn-files-test
  []
  (t/with-fileset [a-edn ["a.edn" {:a "a"}]
                   b-edn ["b.edn" {:b "b"}]]
...
(deftest copy-source-test
  (testing "copy directory"
    (t/with-fileset [directory-source ["src"]
  ...
  (testing "copy jar"
    (t/with-fileset [jar-contents-dir ["contents"]
...
(deftest move-source-files-test
  (t/with-fileset [source-dir ["source"]
                   target-dir ["target"]
...
(deftest explode-classpath-test
  (t/with-fileset [cp-directory ["src"]
